...
from CGG.database.afkdb import get_afk_status
from CGG.database.pmpermitdb import get_approved_users, pm_guard
import CGG.database.welcomedb as CGGdb
import shlex
...
async def welcome_chat(filter, client: Client, message: Message):
    to_welcome = await CGGdb.get_welcome(str(message.chat.id))
    if to_welcome:
        return True
    else:
        return False
